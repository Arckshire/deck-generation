...
screenshots = st.sidebar.file_uploader(
    "Screenshots (PNG/JPG)", type=["png", "jpg", "jpeg"], accept_multiple_files=True
)
...
# Screenshot preview (robust to UploadedFile type + re-use later)
st.subheader("Screenshots (optional)")
if screenshots:
    st.write("Uploaded screenshots:")
    for f in screenshots:
        try:
            img_bytes = f.getvalue()  # safe to reuse later
            img = Image.open(io.BytesIO(img_bytes)).convert("RGB")
            st.image(img, caption=f.name, use_container_width=True)
        except Exception as e:
            st.warning(f"Could not preview {getattr(f, 'name', 'image')}: {e}")
else:
...
if st.button("🚀 Generate PPTX"):
    ...
    def build_slide(title_str, cards_subset, imgs_subset, chart_cfg=None):
        ...
        subtitle = []
        if customer:
            subtitle.append(f"Customer: {customer}")
        if date_range:
            subtitle.append(f"Range: {date_range}")
        if purpose:
            subtitle.append(f"Purpose: {purpose}")
        ...
        placed = 0
        for img_file in imgs_subset[:1]:  # place at most one per slide in this layout
            try:
                img_bytes = img_file.getvalue()  # read bytes fresh; don't rely on internal stream pointer
                pil = Image.open(io.BytesIO(img_bytes)).convert("RGB")
                # Optional: shrink very large images to keep PPT size reasonable
                pil.thumbnail((2560, 1440))
                add_screenshot(slide, pil, img_left, img_top, width_in=img_w)
                placed += 1
            except Exception as e:
                st.warning(f"Could not embed {getattr(img_file, 'name', 'image')}: {e}")
